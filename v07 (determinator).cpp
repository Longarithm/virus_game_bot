...
bool compare4(pair<pii, pair<int, pt> > a, pair<pii, pair<int, pt> > b) {
    if (a.fi.fi != b.fi.fi) return a.fi.fi < b.fi.fi;
    if (a.fi.se != b.fi.se) return a.fi.se < b.fi.se;
    if (a.se.fi != b.se.fi) return a.se.fi < b.se.fi;
    return (a.se.se.x < b.se.se.x || a.se.se.x == b.se.se.x && a.se.se.y < b.se.se.y);
}

int turn, cur_step, matrix[MAXN][MAXN];
bool used[MAXN][MAXN];
string s;
bool no_viruses, time_to_attack, extreme_situation;
...
bool ownage(pt a) {
    forn(j, a.y)
        if (matrix[a.x][j] == 1 || matrix[a.x][j] == 3)
            return 1;
    forn(j, a.x)
        if (matrix[j][a.y] == 1 || matrix[a.x][j] == 3)
            return 1;
    return 0;
}
...
void next_step() {
    ...
    string s;
    pt cur;
    vector<pair<pii, pair<int, pt> > > operations0;
    ...
    forn(i, MAXN)
        forn(j, MAXN) {
            cur = pt(i, j);
            if (matrix[i][j] == 0 && is_near(cur) && !ownage(cur))
                flag = 1;
        }
    if (!flag && cur_step == 0) {
        time_to_attack = 1;
    ...
    forn(i, MAXN)
        forn(j, MAXN) {
            ...
            if (matrix[i][j] == 0 && is_near(cur)) {
                operations0.push_back(make_pair(make_pair(dist_to_live(cur), -count_square(cur)), make_pair(ownage(cur), cur)));
            }        
        }

    random_shuffle(all(operations0));
    sort(all(operations0), compare4);
    ...
    while (pos < int(operations0.size()) - 1) {
        cur = operations0[pos].se.se;
        if (operations0[pos].fi.fi == 1 && !extreme_situation && bad_connect(cur) >= 3 - cur_step)
            pos++;
        else
            break;
    }
    if (operations1.empty() && !operations0.empty()) {
        do_create(operations0[pos].se.se);
        return;
    }
    if (operations1[0].fi > 1 && operations0[pos].fi.fi == 1 && (time_to_attack || cur_step == 0))
        do_create(operations0[pos].se.se);
    else
        do_kill(operations1[0].se);
}
